...
from . import utils, readpaper, data
from .paper_processor import process_papers
from .load_api_keys import load_api_keys_from_files, print_loaded_keys
import threading
import sys
import os
import sys
...
class App(tk.Tk):
    def __init__(self):
        ...
        # --- 系统信息显示 ---
        info_frame = ttk.LabelFrame(controls_frame, text="系统信息" if config.LANGUAGE == 'zh_CN' else "System Info", padding="5")
        info_frame.pack(fill=tk.X, pady=(10, 0))
        
        self.info_label = ttk.Label(info_frame, text="正在加载..." if config.LANGUAGE == 'zh_CN' else "Loading...", 
    ...
    def setup_folder_tab(self):
        ...
        help_button = ttk.Button(apply_frame, text=self.lang["help_button"], command=self.show_help)
    ...
    def preload_info(self):
        ...
        # 更新系统信息显示
        info_text = f"{self.lang['api_key_count']} {api_key_count}\n{self.lang['paper_count']} {paper_count}"
    ...
    def show_help(self):
        ...
        # 插入帮助内容
        help_text.insert(tk.END, self.lang["help_content"])
        help_text.config(state='disabled')  # 设置为只读
        
        # 关闭按钮
        close_button = ttk.Button(help_window, text="关闭" if config.LANGUAGE == 'zh_CN' else "Close", 
